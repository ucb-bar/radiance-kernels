...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify a BFS phase kernel from a sqlite trace."
    )
...
def main() -> int:
    ...
    for symbol, expected_words, actual_words in checks:
        ...
        for index, expected, actual in mismatches[:16]:
            print(
                f"[{index}] expected=0x{expected:08x} ({bits_to_i32(expected)}) "
                f"actual=0x{actual:08x} ({bits_to_i32(actual)})"
            )
