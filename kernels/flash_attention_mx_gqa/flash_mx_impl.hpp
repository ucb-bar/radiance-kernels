...
static __attribute__((noinline)) void fused_softmax_requant_tpr(
    ...
    for (uint32_t row = glane; row < SQ; row += nlanes) {
        ...
        // Pass A: row max AND per-block scaled-S max (registers, NO exp). CAUSAL: a col whose
        // global key (key_offset+col) > q_pos is masked -> -inf (drops out of the max, so it
        // sets neither m_new nor the block's E8M0 scale). key_offset+col comparison is a cheap
        // int compare; apply_mask is warp-uniform (only the diagonal-straddling block sets it).
        _Float16 bSmax[NBLK];
        _Float16 rmax = as_bf16(NEG_INF_BF16_BITS);
        for (uint32_t b = 0; b < NBLK; b++) {
            _Float16 bm = as_bf16(NEG_INF_BF16_BITS);
            for (uint32_t k = 0; k < 32; k++) {
                const uint32_t col = b * 32 + k;
                _Float16 v = (_Float16)(as_bf16(Srow[col]) * scale);
                bool msk = apply_mask && ((key_offset + col) > q_pos);
                v = msk ? as_bf16(NEG_INF_BF16_BITS) : v;      // branchless select
                bm = fmaxf(bm, v);
            }
            bSmax[b] = bm;
            rmax = fmaxf(rmax, bm);
        ...
        // corr = exp(m_old - m_new); first block -> exp(0)=1 (skip SFU; corr unused when first).
        const _Float16 corr = first_block
            ? __builtin_bit_cast(_Float16, ONE_BF16_BITS)
            : mu_fexp((_Float16)(m_old - m_new));

        // Pass B: ONE exp/element (masked -> forced 0), reused for BOTH rowsum and e4m3 requant.
        // E8M0 scale from exp(bSmax - m_new) (1 exp/block); clamp se<=0 because a FULLY-masked
        // block has bSmax=-inf -> exp(-inf)=NaN/0 -> floor_log2 could yield a large +se -> E8M0
        // 2^se overflows to +inf in the mesh and 0*inf NaNs the systolic PV (matches cooperative).
        _Float16 rowsum = (_Float16)0;
        const uint32_t ti = row / 16, rr = row % 16;
        for (uint32_t b = 0; b < NBLK; b++) {
            const uint32_t base = b * 32;
            const _Float16 bPmax = mu_fexp((_Float16)(bSmax[b] - m_new));  // 1 exp/block
            ...
            for (uint32_t w = 0; w < 32 / 4; w++) {   // 4 e4m3 per word, tiled store
                ...
                _Float16 e[4];
                for (uint32_t k = 0; k < 4; k++) {    // issue the 4 exps back-to-back (SFU pipe)
                    const uint32_t col = col0 + k;
                    const _Float16 s = (_Float16)(as_bf16(Srow[col]) * scale);
                    _Float16 ev = mu_fexp((_Float16)(s - m_new));         // 1 exp/element
                    bool msk = apply_mask && ((key_offset + col) > q_pos);
                    e[k] = msk ? (_Float16)0 : ev;    // masked prob = 0 (excl. sum/PV/e4m3)
                }
                uint32_t packed = 0;
                for (uint32_t k = 0; k < 4; k++) {
                    rowsum = (_Float16)(rowsum + e[k]);
                    packed |= (uint32_t)bf16_to_e4m3_scaled(
                                  __builtin_bit_cast(uint16_t, e[k]), se) << (8 * k);
                }
...
static __attribute__((noinline)) void fused_softmax_requant(
    ...
    for (uint32_t row = warp; row < SQ; row += nwarps) {
        ...
        for (uint32_t c = 0; c < CPL; c++) {
            ...
            bool m = apply_mask && ((key_base + c) > q_pos);   // causal: key_pos > query_pos
            masked[c] = m;
