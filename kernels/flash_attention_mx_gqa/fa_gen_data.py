...
def main():
    ...
    # ---- goldens from the verified model (full Sk; masked blocks contribute nothing) ----
    O_ref = FA.attention_reference_causal_gqa(Q, K, V, softmax_scale, q_pos0)          # fp32
    O_mx = FA.mx_attention_flash_causal_gqa(Q, K, V, softmax_scale, q_pos0, Bk)        # MX flash
    ...
    np.save(os.path.join(_od, "golden_O_gqa_mx_u16.npy"), u16_codes(O_mx.reshape(n_q * Sq, d)))
    np.save(os.path.join(_od, "golden_O_gqa_ref_u16.npy"), u16_codes(O_ref.reshape(n_q * Sq, d)))
