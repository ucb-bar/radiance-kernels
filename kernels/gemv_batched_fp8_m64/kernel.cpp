...
//   M = batch of decode steps (or heads) = 64.
//
...
//
// The tile is NON-SQUARE (TILE_M=64, TILE_N=128). The stock mxgemm_lib only
// allowed square tiles; this copy differentiates the A/B scale-factor counts
// (SCALE_FACTORS_PER_TILE_A/_B) so TILE_M != TILE_N verifies correctly.
//
// This variant is fixed at M=64 (fp8); the committed `data` carries that shape.
...
constexpr GemmConfig GEMM_CFG{ .TILE_M=MATMUL_M, .TILE_N=MATMUL_N, .TILE_K=128, .DATATYPE=GemmDatatype::FP8, .QUANT_OUTPUT=false };
