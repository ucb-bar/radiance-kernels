...
# ===================== Gemma-2 extension: attn logit soft-cap + sliding window =========
# Two Gemma-2 attention deltas vs plain causal GQA (both confirmed from
# google/gemma-2-2b-it config.json):
#   attn_logit_softcapping = 50.0  ->  each QK^T score s is replaced by cap*tanh(s/cap)
#                                      BEFORE the softmax (and, in flash form, BEFORE the
#                                      running-max/exp update -- the wrinkle below).
#   sliding_window = 4096 on the EVEN layers (layer_idx % 2 == 0). A sliding-window layer
#                    lets a query at position q attend only to keys in (q-window, q]
#                    (still causal on the high side). The ODD layers are full-causal.
# query_pre_attn_scalar = 256 = head_dim, so the query pre-scale is IDENTICAL to the
# standard 1/sqrt(head_dim) already applied as softmax_scale -> no separate op.

def softcap(x, cap):
    """Gemma soft-cap: cap * tanh(x / cap) in fp32 (the true-math reference)."""
    if cap is None:
        return x
    return cap * torch.tanh(x / cap)


def softcap_hw(x, cap):
    """bf16 soft-cap mirroring the kernel's bf16_softcap (flash_mx_impl.hpp): tanh via the
    stable exp(-2|x|) form, every op in bf16. This is what the MX-FLASH golden uses, so the
    kernel (which also computes tanh in bf16 with the hardware fexp) reproduces it tightly --
    otherwise the fp32-vs-bf16 tanh gap flips arg-maxes in the peaked, capped softmax rows."""
    if cap is None:
        return x
    bf = torch.bfloat16
    inv = torch.tensor(1.0 / cap, dtype=bf).float()
    xx = (x.to(bf).float() * inv)
    xx = xx.to(bf).float()
    ax = xx.abs()
    e = torch.exp(-2.0 * ax).to(bf).float()
    t = ((1.0 - e) / (1.0 + e)).to(bf).float()
    out = cap * torch.where(xx < 0, -t, t)
    return out.to(bf).float()


def gemma_bool_mask(Sq, Sk, q_pos0, window=None):
    """True where masked. Causal (key_pos > query_pos) OR, if a sliding window is set,
    key_pos <= query_pos - window (too old)."""
    qpos = q_pos0 + torch.arange(Sq).unsqueeze(1)      # [Sq,1]
    kpos = torch.arange(Sk).unsqueeze(0)               # [1,Sk]
    m = kpos > qpos                                    # causal high side
    if window is not None:
        m = m | (kpos <= (qpos - window))              # sliding-window low side
    return m


def attention_reference_gemma(Q, K, V, softmax_scale, q_pos0, attn_softcap=None, window=None):
    """Pure fp32 GQA attention reference WITH Gemma soft-cap + optional sliding window.
    Order matches Gemma2Attention: scale -> soft-cap -> mask -> softmax -> PV."""
    n_q, Sq, d = Q.shape
    n_kv, Sk, _ = K.shape
    grp = n_q // n_kv
    mask = gemma_bool_mask(Sq, Sk, q_pos0, window)
    outs = []
    for h in range(n_q):
        kv = h // grp
        S = softmax_scale * (Q[h] @ K[kv].t())         # [Sq,Sk]
        S = softcap(S, attn_softcap)                    # cap BEFORE the mask/softmax
        S = S.masked_fill(mask, float("-inf"))
        P = torch.softmax(S, dim=1)
        outs.append(P @ V[kv])
    return torch.stack(outs)
...
def mx_attention_flash_gemma(Q, K, V, softmax_scale, q_pos0, block_n,
                             attn_softcap=None, window=None):
    """Streaming online-softmax MX GQA attention with the Gemma-2 soft-cap folded into the
    online softmax and an optional sliding-window mask. Mirrors the kernel EXACTLY.

    THE WRINKLE (soft-cap fold): the cap is applied to each PARTIAL score tile Sj right after
    the softmax-scale and BEFORE the block max / running-max / exp. Because tanh is a
    *nonlinear monotonic* map, the running max m_run carried across key blocks is a max of
    already-capped scores; if a later block computed its block-max on UN-capped scores the
    cross-block max compare (m_new = max(m_run, bmax)) would mix two different score scales
    and pick the wrong m_new -> a wrong exp reference -> wrong probabilities. Applying the cap
    up front makes every score (and hence every max, corr=exp(m_old-m_new) and exp(s-m_new))
    live on one consistent capped scale, so the standard flash recurrence is reused verbatim.
    """
    n_q, Sq, d = Q.shape
    n_kv, Sk, _ = K.shape
    grp = n_q // n_kv
    nbu = blocks_used(Sq, Sk, q_pos0, block_n)         # leading blocks (causal high-side skip)
    qpos = q_pos0 + torch.arange(Sq).unsqueeze(1)      # [Sq,1]
    outs = []
    for h in range(n_q):
        kv = h // grp
        ...
        for j in range(nbu):
            ...
            Sj_raw = mx_gemm(Qf8, Qs, Kjf8.t().contiguous(), Kjs.t().contiguous())  # [Sq,Bk]
            Sj = q_bf16(Sj_raw * softmax_scale)
            Sj = q_bf16(softcap_hw(Sj, attn_softcap))  # <-- soft-cap BEFORE max/exp (the wrinkle)
            kpos = koff + torch.arange(block_n).unsqueeze(0)    # [1,Bk]
            mblk = kpos > qpos                                  # causal high side
            if window is not None:
                mblk = mblk | (kpos <= (qpos - window))         # sliding-window low side
            Sj = Sj.masked_fill(mblk, float("-inf"))
            first = (j == 0)
            bmax = Sj.amax(dim=1, keepdim=True)
            m_new = bmax if first else torch.maximum(m_run, bmax)
            corr = torch.ones_like(bmax) if first else q_bf16(torch.exp(m_run - m_new))
            Pj = q_bf16(torch.exp(Sj - m_new))
            Pj = torch.where(mblk, torch.zeros_like(Pj), Pj)    # masked -> exactly 0
            lsum = q_bf16(Pj.sum(dim=1, keepdim=True))
            l_run = q_bf16((torch.zeros_like(l_run) if first else l_run * corr) + lsum)
            Pjf8, Pjs = mx_quantize_cols(Pj)
            Vjt_f8, Vjt_s = mx_quantize_cols(Vj.t().contiguous())
