...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify softmax output by reconstructing x_raw from a sqlite dmem trace."
    )
    parser.add_argument("trace_db", type=Path, help="Path to Cyclotron/RTL sqlite trace database")
    parser.add_argument("elf", type=Path, help="Path to softmax kernel radiance ELF")
    parser.add_argument(
        "--expected",
        type=Path,
    ...
    parser.add_argument(
        "--symbol",
        default="x_raw",
        help="Output symbol to resolve with llvm-nm (default: x_raw)",
    )
...
def parse_expected_halfwords(expected_path: Path) -> list[int]:
    ...
    match = re.search(
        r"const\s+uint16_t\s+expected_softmax_raw\[\]\s*=\s*\{([^}]*)\};",
        text,
        re.S,
    )
    if match is None:
        raise ValueError("expected_softmax_raw[] not found in expected file")
    ...
    if not values:
        raise ValueError("expected_softmax_raw[] is empty")
    return values
