...
// Map:  0x40050000 kernel MARK array (flash_attention_mx.cpp, runtime index -> already exact)
...
#ifdef FA_SCALE_GUARD
// OVERRUN DETECTOR, gated -- default build below is UNCHANGED (29 files include this header).
// The array is sized 128*128/32 = 512 words with a TODO flagging the hardcode.  FA's largest C
// tile is TILE_M x TILE_N = 64 x 256 = 16384 elements = 512 scale factors at 32-element MX blocks
// -- an EXACT fit with ZERO margin.  In FA's image the array sits at 0x10001060..0x10001860 and
// schedule_context at 0x10001880: a 32-BYTE GAP.  So >8 words of overrun corrupts the launch
// callback WHILE THE KERNEL RUNS, which is exactly what Mode A looks like.
// The element count above is computed from the C DECLARATION, not from what Gemmini actually
// emits -- so measure the hardware instead of trusting the arithmetic.
// A struct forces adjacency: -fdata-sections gives NO ordering guarantee between separate arrays,
// so a bare guard[] could land anywhere.  The guard is NON-ZERO initialised, which puts the whole
// struct in .data rather than the orphaned .bss -- so the sentinel is genuinely LOADED and
// trustworthy, where the current array is never initialised at all.
#define FA_GUARD_WORD 0x5A5AC0DEu
static struct {
    uint32_t scales[128 * 128 / 32];
    uint32_t guard[64];
} C_scale_block __attribute__((aligned(32))) = {
    { 0 },
    { FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD,
      FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD,
      FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD,
      FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD,
      FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD,
      ...
      FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD,
      FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD,
      FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD, FA_GUARD_WORD }
};
#define C_scale_factors (C_scale_block.scales)
#else
