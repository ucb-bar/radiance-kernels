...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify sfilter output by reconstructing dst_raw from a Cyclotron sqlite trace."
    )
    parser.add_argument("trace_db", type=Path, help="Path to Cyclotron sqlite trace database")
    parser.add_argument("elf", type=Path, help="Path to sfilter kernel.radiance.elf")
    parser.add_argument(
        "--expected",
        type=Path,
    ...
    parser.add_argument(
        "--symbol",
        default="dst_raw",
        help="Output symbol to resolve with llvm-nm (default: dst_raw)",
    )
...
def parse_expected_words(expected_path: Path) -> list[int]:
    ...
    match = re.search(
        r"const\s+uint32_t\s+expected_sfilter_raw\[\]\s*=\s*\{([^}]*)\};",
        text,
        re.S,
    )
    if match is None:
        raise ValueError("expected_sfilter_raw[] not found in expected file")
    ...
    if not values:
        raise ValueError("expected_sfilter_raw[] is empty")
    return values
