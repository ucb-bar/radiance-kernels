...
int main() {
  ...
  // Estimate ALU throughput using independent dependency chains
  ...
  for (uint32_t i = 0; i < iterations; ++i) {
    // Each lane is independent to expose completion bandwidth
    a0 += 3u; a1 += 5u; a2 += 7u; a3 += 11u;
    a4 += 13u; a5 += 17u; a6 += 19u; a7 += 23u;
  }
  ...
  tune_print_result("execute_throughput_alu", result);
  return 0;
}
