...
constexpr uint32_t kLatencyFootprintWords = 64;   // 256B
constexpr uint32_t kBandwidthFootprintWords = 128; // 512B
constexpr uint32_t kStrideWords = 1;
...
int main() {
  ...
  uint32_t acc = 0x10203040u;
  uint32_t idx = 0u;
  for (uint32_t i = 0; i < kWarmupIterations; ++i) {
    idx = (idx + kStrideWords + (acc & 3u)) & (kLatencyFootprintWords - 1u);
    acc ^= src[idx];
  }
  ...
  uint32_t p0 = 0u, p1 = 8u, p2 = 16u, p3 = 24u;
  uint32_t p4 = 32u, p5 = 40u, p6 = 48u, p7 = 56u;
  ...
  tune_print_result("gmem_l0_latency_bw", result);
  return 0;
}
