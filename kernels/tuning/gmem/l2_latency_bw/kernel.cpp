...
constexpr uint32_t kLatencyFootprintWords = 8192;    // 32KB
constexpr uint32_t kBandwidthFootprintWords = 32768; // 128KB
...
int main() {
  ...
  uint32_t acc = 0xA5A55A5Au;
  uint32_t idx = 0u;
  for (uint32_t i = 0; i < kWarmupIterations; ++i) {
    idx = (idx + kStrideWords + (acc & 15u)) & (kLatencyFootprintWords - 1u);
    acc ^= src[idx];
  }
  ...
  tune_print_result("gmem_l2_latency_bw", result);
  return 0;
}
