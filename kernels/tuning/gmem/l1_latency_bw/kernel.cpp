...
constexpr uint32_t kLatencyFootprintWords = 2048;   // 8KB
constexpr uint32_t kBandwidthFootprintWords = 4096; // 16KB
constexpr uint32_t kStrideWords = 4;
...
int main() {
  ...
  uint32_t acc = 0x55667788u;
  uint32_t idx = 0u;
  for (uint32_t i = 0; i < kWarmupIterations; ++i) {
    idx = (idx + kStrideWords + (acc & 7u)) & (kLatencyFootprintWords - 1u);
    acc ^= src[idx];
  }
  ...
  uint32_t p0 = 0u, p1 = 64u, p2 = 128u, p3 = 192u;
  uint32_t p4 = 256u, p5 = 320u, p6 = 384u, p7 = 448u;
  ...
  tune_print_result("gmem_l1_latency_bw", result);
  return 0;
}
