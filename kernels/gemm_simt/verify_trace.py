...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify gemm_simt output by reconstructing C_raw from a sqlite dmem trace."
    )
    parser.add_argument("trace_db", type=Path, help="Path to RTL/Cyclotron sqlite trace database")
    parser.add_argument("elf", type=Path, help="Path to gemm_simt kernel.radiance.elf")
    parser.add_argument(
        "--expected",
        type=Path,
...
def parse_expected_halfwords(expected_path: Path) -> list[int]:
    ...
    match = re.search(
        r"const\s+uint16_t\s+expected_C_raw\[\]\s*=\s*\{([^}]*)\};",
        text,
        re.S,
    )
    if match is None:
        raise ValueError("expected_C_raw[] not found in expected file")
    ...
    if not values:
        raise ValueError("expected_C_raw[] is empty")
    return values
