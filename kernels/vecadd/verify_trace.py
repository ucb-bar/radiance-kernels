...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify vecadd output by reconstructing C_raw from a Cyclotron sqlite trace."
    )
    parser.add_argument("trace_db", type=Path, help="Path to Cyclotron sqlite trace database")
    parser.add_argument("elf", type=Path, help="Path to vecadd kernel.radiance.elf")
    parser.add_argument(
        "--expected",
        type=Path,
...
def parse_expected_words(expected_path: Path) -> list[int]:
    ...
    match = re.search(
        r"const\s+uint32_t\s+expected_vecadd_raw\[\]\s*=\s*\{([^}]*)\};",
        text,
        re.S,
    )
    if match is None:
        raise ValueError("expected_vecadd_raw[] not found in expected file")
    ...
    if not values:
        raise ValueError("expected_vecadd_raw[] is empty")
    return values
